...
Splash::Splash(GameManager* owner) :
    State(owner)
{
    State::initialize();
}
...
void Splash::handle(const skEventType& evt)
{
...
void Splash::update()
{
    ...
    SKint32 w, h;
    skGetFontTextExtent(res.Font, R::Title.buf, R::Title.len, &w, &h);

    pos.x = sz.x * 0.5f - (skScalar)w * 0.5f;
    pos.y = sz.y * 0.5f - (skScalar)h * 0.5f;
    pos.y -= skScalar(R::menuTextSize >> 1);

    RU::displayDropShadow(res, pos.x, pos.y, R::Title);
}
